...
const HUBS = [
  ["berger", "Berger Junction", 9.0667, 7.4431],
  ["wuse", "Wuse Market", 9.0765, 7.4623],
  ["area1", "Garki Area 1", 9.0358, 7.4881],
  ["garki", "Garki Market", 9.0304, 7.4898],
  ["central", "Central Area", 9.0556, 7.4914],
  ["jabi", "Jabi Park", 9.0655, 7.4207],
  ["utako", "Utako Motor Park", 9.0679, 7.4498],
  ["mabushi", "Mabushi", 9.0834, 7.4534],
  ["gwarinpa", "Gwarinpa", 9.1098, 7.4055],
  ["kubwa", "Kubwa", 9.1544, 7.322],
  ["lugbe", "Lugbe", 8.9778, 7.3724],
  ["airportRd", "Airport Road Junction", 9.0038, 7.3957],
  ["nyanya", "Nyanya", 9.0251, 7.5681],
  ["mararaba", "Mararaba", 9.0312, 7.5885],
  ["asokoro", "Asokoro", 9.0361, 7.5247],
  ["maitama", "Maitama", 9.0873, 7.4934],
  ["apo", "Apo", 8.9898, 7.4992],
  ["karu", "Karu", 9.0098, 7.5695],
  ["lifeCamp", "Life Camp", 9.0805, 7.3908],
...
const LINKS = [
  ...
  ["jabi", "lifeCamp", "keke", 250, 350, 8],
  ["lifeCamp", "gwarinpa", "bus", 350, 500, 12],
  ["gwarinpa", "dutse", "keke", 250, 400, 10],
  ["dutse", "kubwa", "bus", 400, 600, 18],
  ["berger", "wuse", "bus", 400, 550, 14],
  ["wuse", "central", "bus", 300, 450, 10],
  ["central", "area1", "bus", 300, 450, 11],
  ["area1", "garki", "keke", 200, 300, 6],
  ["garki", "apo", "keke", 250, 400, 10],
  ["wuse", "mabushi", "keke", 250, 350, 8],
  ["mabushi", "maitama", "taxi", 350, 500, 9],
  ["maitama", "central", "taxi", 350, 550, 10],
  ["central", "asokoro", "taxi", 400, 650, 12],
  ["asokoro", "karu", "taxi", 450, 700, 15],
  ["karu", "nyanya", "bus", 250, 400, 9],
  ["nyanya", "mararaba", "bus", 300, 500, 11],
  ["berger", "airportRd", "bus", 400, 600, 15],
  ["airportRd", "lugbe", "bus", 350, 550, 14],
  ["airportRd", "apo", "taxi", 450, 650, 14],
  ["apo", "area1", "keke", 250, 400, 9],
  ["utako", "mabushi", "keke", 250, 400, 8],
  ["jabi", "gwarinpa", "bus", 350, 500, 14],
  ["wuse", "maitama", "taxi", 300, 500, 10],
  ["garki", "asokoro", "taxi", 350, 550, 11],
  ["area1", "karu", "bus", 450, 650, 15],
  ["central", "nyanya", "bus", 500, 750, 20],
  ["berger", "central", "bus", 450, 650, 16],
  ["kubwa", "gwarinpa", "bus", 450, 650, 18],
  ["lugbe", "apo", "bus", 450, 650, 17],
  ["mabushi", "central", "bus", 300, 450, 10],
].map((e, i) => ({
  id: `route-${i + 1}`,
  a: e[0],
  b: e[1],
...
const state = {
  ...
  voice: true,
  paused: false,
  voiceName: localStorage.getItem("alongVoice") || "browser",
  routeLayer: null,
  markerLayer: null,
};
const map = L.map("map", { zoomControl: true, minZoom: 9 }).setView(
  [9.0556, 7.4914],
  12,
);
const tileUrl = [
...
const iconHtml = (color = "#171817") =>
  L.divIcon({
    className: "along-marker-wrap",
    html: `<span class="along-marker" style="--marker:${color}"><i></i></span>`,
    iconSize: [28, 28],
    iconAnchor: [14, 14],
  });
LINKS.forEach((e) =>
  L.polyline(
    ...
    ],
    {
      ...
      weight: 3,
      opacity: 0.2,
      dashArray: e.mode === "taxi" ? "5 8" : null,
    },
  ).addTo(networkLayer),
);
HUBS.forEach((h) =>
  L.marker([h.lat, h.lng], { icon: iconHtml("#171817") })
...
L.control
  .layers(
    null,
    { "Prototype transport routes": networkLayer, "Transport hubs": hubLayer },
...
function buildJourney(origin, destination, preference, label) {
  ...
  edges.forEach((e, i) => {
    ...
    legs.push({
      ...
      text: `Take a ${e.mode} going to ${dir}. Drop at ${t.name}.`,
      coords: [
        [f.lat, f.lng],
        [t.lat, t.lng],
      ],
  ...
  if (!legs.length)
    legs.push({
      ...
      from: origin.name,
      to: destination.name,
      distance: Math.round(km(origin, destination) * 1000),
      time: Math.round(km(origin, destination) * 12),
  ...
  return {
    ...
    fare: [
      fares.reduce((s, l) => s + l.fare[0], 0),
      fares.reduce((s, l) => s + l.fare[1], 0),
    ],
    walk,
    transfers: changes,
...
async function geocode(query) {
  ...
  try {
    ...
    if (r.ok) {
      const items = (await r.json()).map((x) => ({
        name: x.display_name,
        lat: +x.lat,
        lng: +x.lon,
        type: "search",
...
async function reverseGeocode(lat, lng) {
  try {
    const u = new URL("/api/geocode", location.origin);
    u.search = new URLSearchParams({ lat, lon: lng });
    const r = await fetch(u);
    if (r.ok) {
      const x = await r.json();
...
function setupSearch(input, resultEl, key) {
  input.addEventListener("input", () => {
    ...
    const local = localMatch(input.value);
    renderSuggestions(local, resultEl, input, key);
    if (input.value.trim().length < 3) return;
    searchTimer = setTimeout(async () => {
      try {
        const remote = await geocode(input.value);
        renderSuggestions(
          [...local, ...remote].slice(0, 6),
          resultEl,
          input,
          key,
        );
        ...
        if (!local.length)
          showToast(
            "Online place search is unavailable. Pick from the map instead.",
          );
      }
    }, 900);
  });
  input.addEventListener("focus", () => {
    const local = localMatch(input.value);
    if (local.length) renderSuggestions(local, resultEl, input, key);
  });
}
function renderSuggestions(items, el, input, key) {
  el.innerHTML = items
    .map(
      (x, i) =>
        `<button type="button" data-i="${i}">${x.name}<small>${x.type === "search" ? "OpenStreetMap place" : "Along transport hub"}</small></button>`,
    )
    .join("");
  ...
  [...el.children].forEach(
    (b, i) =>
      (b.onclick = () => {
        ...
        el.hidden = true;
        map.flyTo([items[i].lat, items[i].lng], 14);
      }),
  );
}
async function resolveInput(input, key) {
  if (state[key]) return state[key];
  const local = HUBS.find(
    (h) => h.name.toLowerCase() === input.value.trim().toLowerCase(),
  );
  if (local) return (state[key] = local);
  const found = await geocode(input.value);
  if (!found[0]) throw Error(`We couldn't find ${input.value} in Abuja.`);
...
async function submitRoute() {
  ...
  try {
    const [o, d] = await Promise.all([
      resolveInput(originInput, "origin"),
      resolveInput(destinationInput, "destination"),
    ]);
    if (km(o, d) < 0.05) throw Error("Choose two different places.");
    ...
    $("#resultsTitle").textContent = `${o.name} to ${d.name}`;
    renderTabs();
    renderRoute();
  } catch (e) {
    showToast(e.message || "We could not build this route.");
  } finally {
    label.textContent = "Find my route";
    btn.disabled = false;
  }
}
function renderTabs() {
  const el = $("#routeTabs");
  el.innerHTML = state.routes
    .map(
      (r, i) =>
        `<button class="route-tab ${i === state.selected ? "active" : ""}" data-route="${i}"><b>${r.label}</b><small>${r.time} min · ${money(r.fare[0])}+</small></button>`,
    )
    .join("");
  $$("[data-route]").forEach(
    (b) =>
      (b.onclick = () => {
        state.selected = +b.dataset.route;
        renderTabs();
        renderRoute();
      }),
  );
  ...
  r.legs.forEach((l) => {
    ...
    group.push(
      L.polyline(l.coords, {
        ...
        dashArray: l.mode === "walk" ? "3 10" : null,
        opacity: 0.95,
      }),
    );
  });
  group.push(
    L.marker([r.origin.lat, r.origin.lng], {
      icon: iconHtml("#b8f04a"),
    }).bindTooltip("Start"),
  );
  group.push(
    L.marker([r.destination.lat, r.destination.lng], {
      icon: iconHtml("#8a72e8"),
...
function updateNav() {
  const r = state.routes[state.selected],
    l = r.legs[state.navStep];
  if (!l) {
    $("#navInstruction").textContent = "You've arrived";
    $("#navDistance").textContent = r.destination.name;
    $("#navModeIcon").textContent = "✓";
    $("#navStepNumber").textContent = "COMPLETE";
    $("#navProgress").style.width = "100%";
    $("#nextStepBtn").textContent = "Trip complete";
    speak(`You've arrived at ${r.destination.name}.`);
    return;
  }
  $("#navStepNumber").textContent = `${state.navStep + 1} OF ${r.legs.length}`;
  $("#navInstruction").textContent = l.text;
  $("#navDistance").textContent = l.distance
    ? `${l.distance} metres away`
    : `About ${l.time} minutes`;
  $("#navModeIcon").textContent = modeIcon(l.mode);
...
async function speak(text) {
  ...
  if (cloud)
    showToast(
      "Online Nigerian voice is not configured; using your device voice.",
    );
}
...
setupSearch(originInput, $("#originResults"), "origin");
setupSearch(destinationInput, $("#destinationResults"), "destination");
$("#swapBtn").onclick = () => {
...
$$("[data-trip]").forEach(
  (b) =>
    (b.onclick = () => {
      const [a, d] = b.dataset.trip.split("|");
      originInput.value = a;
      destinationInput.value = d;
      state.origin = HUBS.find((h) => h.name === a) || null;
      state.destination = HUBS.find((h) => h.name === d) || null;
      submitRoute();
    }),
);
$$("[data-pick]").forEach(
  (b) =>
    (b.onclick = () => {
      ...
      $("#pickTarget").textContent = state.pickMode;
      $("#pickBanner").hidden = false;
      map.getContainer().classList.add("picking");
      showToast(`Tap the map to set ${state.pickMode}`);
    }),
);
$("#cancelPick").onclick = () => {
  state.pickMode = null;
...
map.on("click", async (e) => {
  ...
  const key = state.pickMode,
    input = key === "origin" ? originInput : destinationInput;
  input.value = "Dropped pin…";
  const name = await reverseGeocode(e.latlng.lat, e.latlng.lng);
  state[key] = { name, lat: e.latlng.lat, lng: e.latlng.lng, type: "pin" };
  input.value = name;
  state.pickMode = null;
  $("#pickBanner").hidden = true;
  map.getContainer().classList.remove("picking");
  L.marker(e.latlng, {
    icon: iconHtml(key === "origin" ? "#b8f04a" : "#8a72e8"),
  })
    .addTo(map)
...
$("#locateBtn").onclick = () => {
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    () =>
      showToast(
        "We couldn't access your location. Search or pick from the map.",
      ),
...
$("#nextStepBtn").onclick = () => {
  ...
  const l = state.routes[state.selected].legs[state.navStep];
  if (!l) return;
  state.navStep++;
  updateNav();
  const next = state.routes[state.selected].legs[state.navStep];
  if (next) speak(`You've reached ${l.to}. ${next.text}`);
};
$("#pauseBtn").onclick = () => {
...
$("#voiceBtn").onclick = () => {
  ...
  showToast(`Voice ${state.voice ? "on" : "off"}`);
};
$("#closeNavBtn").onclick = () => {
...
$("#voiceSelect").onchange = (e) => {
  ...
  $("#settingsVoice").value = state.voiceName;
  localStorage.setItem("alongVoice", state.voiceName);
};
$("#settingsBtn").onclick = () => {
  $("#settingsVoice").value = state.voiceName;
  $("#settingsDialog").showModal();
};
$("#settingsVoice").onchange = (e) => {
  state.voiceName = e.target.value;
  $("#voiceSelect").value = state.voiceName;
  localStorage.setItem("alongVoice", state.voiceName);
};
$("#reportBtn").onclick = () => $("#reportDialog").showModal();
